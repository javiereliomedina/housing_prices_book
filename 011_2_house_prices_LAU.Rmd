...
res_prices <- future_map_dfr(.x = csv_files_path,
...
dk_muni_prices <- dk_muni_pop %>% 
  ...
  pivot_wider(names_from = ancestry,
              values_from = c(pop, pop_dif_2008, pop_pct_2008, pop_km2)) %>% 
...
Plots variation over time (adjusted prices)

```{r fig-price-change-muni, fig.height=25, fig.width=10, fig.cap = "Adjusted median house price (kDKK/m2) change from 2008"}

# Short municipalities b population (e.g same order than pop_muni)
lvls <- levels(pop_muni$muni_name)
dk_muni_prices <- dk_muni_prices %>% 
  mutate(muni_name = factor(muni_name, levels = lvls))

# Plot
ggplot() +
  geom_line(data = dk_muni_prices,
            aes(x = year,
...
```{r fig-prices-pop,  fig.height=7, fig.width=10, fig.cap = "Association between population density and median housing prices at municipality level"}

# Big cities/urban areas
  lbls <- c("København", "Aarhus", "Odense", "Aalborg", "Frederiksberg")
  dk_muni_prices_lbls <- dk_muni_prices %>% filter(muni_name %in% lbls)
...
```{r fig-prices-LAUs, animation.hook="gifski", fig.cap = "Adjusted 2019 housing prices variation (baseline 2008)"}

# Make intervals
brks <- dk_muni_prices$median_2019_kDKK_m2_dif %>% quantile(seq(0,1,0.1),na.rm = T)
dk_muni_prices <- dk_muni_prices %>% 
  mutate(median_2019_kDKK_m2_dif_brk = cut(dk_muni_prices$median_2019_kDKK_m2_dif,
                                           breaks = brks,
                                           include.lowest = TRUE))
...
f_plot <- function(df) {
  ggplot() +
    geom_sf(data = df,
            aes(fill = median_2019_kDKK_m2_dif_brk),
    ...
    #                      midpoint = 0,
    #                      limits = c(-100, 300)) +
    scale_fill_brewer(name = "Change [%]",
                      palette = "RdYlBu",
                      direction = -1,
                      drop = FALSE) +
    my_theme_map() +
    labs(subtitle = paste("Date", df$year, sep = ": "),
         x = "",
         y = "")
}
...
# Animation
for(i in seq_along(plts)) { print(plts[[i]]) }
