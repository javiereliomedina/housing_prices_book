...
Price change with 2008 as baseline (year we start to have data of population change).

```{r}

res_prices_muni <- res_prices_muni %>% 
  filter.(year >= 2008) %>% 
...
dk_muni_prices <- dk_muni %>% 
  left_join.(res_prices_muni, by = "muni_id") %>% 
  st_sf()
