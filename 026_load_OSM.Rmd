...
We get location information from [Open Street Map](https://wiki.openstreetmap.org/wiki/Map_features). We select services inside a buffer of approx. 2 km around the study area (Figure \@ref(fig:fig-osm-poly) and \@ref(fig:fig-osm-points)), assuming that people may use then (e.g. parks, parking areas) outside Copenhagen City but they are close enough to the people houses. 

## Features

### Areal features

The selected features, and their expected effect on housing prices, are reported in Table \@ref(tab:tbl-osm-poly). The mass of the service ($M_j$) for the potential model is the area (in $m^2$) of the feature, and we estimate the distance to the grid cells of 100m x 100m from the centroid of the polygon. 
...
Point features and their expected effect on housing prices (Table \@ref(tab:tbl-osm-points)). In this case, the mass of the service ($M_j$) for the potential model is the same for all points (i.e. 1).  
...
Parameters for each OSM feature (Table \@ref(tab:tbl-sif-pot-model))
```{r tbl-sif-pot-model}
