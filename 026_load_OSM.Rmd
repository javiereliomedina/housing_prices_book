...
```{r fig-osm-data, cache=TRUE, dependson="tbl-osm-data", fig.cap="OMS data"}
...
oms_points_dnld <- oe_get(place = "Copenhagen",
            layer = "points",
...
oms_points <-  merge(oms_points_list, oms_points_dnld, by = c("key", "value")) %>% 
  # Convert to sf object
  st_sf() %>% 
  # Transform CRS
  st_transform("EPSG:25832")
...
```{r, map-osm-points, fig.cap="OSM point data"}

mapview::mapview(oms_points, zcol = "feature") 
