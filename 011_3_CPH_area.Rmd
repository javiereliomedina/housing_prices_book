...
res_units_oft_g100m <- res_units_oft %>%
  ...
  summarise(n_runits = n(), 
            mean_kDKK_m2   = mean(price_2019_kDKK_m2, na.rm = TRUE),
            median_kDKK_m2 = median(price_2019_kDKK_m2, na.rm = TRUE)) %>% 
  ungroup()
...
prices_g100m <- pop_g100m %>% 
  ...
  rename(year = date) %>% 
  right_join(res_units_oft_g100m, by = c("year", "grid_ID"))
...
f_plot <- function(df) {
  ggplot() +
    geom_sf(data = study_area, fill = "grey") +
    geom_sf(data = df, 
            aes(fill = cut(median_kDKK_m2,
                           breaks = quantile(prices_g100m$median_kDKK_m2,
                                             seq(0, 1, 0.1)), 
                           include.lowest = TRUE, 
                           right = TRUE, 
                           dig.lab = 0,
