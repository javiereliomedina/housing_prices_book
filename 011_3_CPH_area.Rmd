...
```{r runits-clean-cph}
  ...
  res_units_oft <- res_units %>%
    ...
    filter.(price_2019_kDKK > 0)
    
```

The total number of residential units used for the analysis is therefore `r nrow(res_units_oft)` (Table \@ref(tab:tbl-runits-clean-cph)).
```{r tbl-runits-clean-cph}
...
prices_g100m <- pop_g100m %>% 
  select(-pop_total_km2_cut) %>% 
  rename(year = date) %>% 
  right_join(sum_runits_prices, by = c("year", "grid_ID"))

```

```{r fig-prices-runits-grid-a, animation.hook="gifski", fig.cap = "Median 2019-adjusted house prices in the ordinary free trade by grid cells of 100m x 100m"}
...
```{r fig-prices-runits-grid-b, fig.cap = "Median 2019-adjusted house prices in the ordinary free trade by grid cells of 100m x 100m"}
...
```{r bivariate-map, animation.hook="gifski", fig.cap = "Population density and house prices in the ordinary free trade"}
...
```{r bivariate-map-b, fig.height=10, fig.width=10, fig.cap = "Population density and house prices in the ordinary free trade"}
