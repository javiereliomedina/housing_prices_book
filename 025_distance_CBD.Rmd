...
Time in public transport from [OpenTripPlanner](https://www.opentripplanner.org/), and the r-package [opentripplanner](https://github.com/ropensci/opentripplanner) (@Morgan2019). 
...
# 
# ## Copenhagen pbf (get from oe_get)
# oe_get(place = "Copenhagen",
#        provider = "bbbike",
...
# file.remove("./otp/graphs/default/bbbike_Copenhagen.gpkg")
# 
# Open Trip Planner
...
## Isochone
isochrone_pt  <- otp_isochrone(
  otpcon = otpcon,
  fromPlace = c(12.5860, 55.6805),
  mode = c("WALK","TRANSIT"),
  maxWalkDistance = 1000,
  date_time = as.POSIXct(strptime("2021-05-11 08:00", "%Y-%m-%d %H:%M")),
  cutoffSec = c(15, 30, 45, 60, 75, 90) * 60) %>%
  # Transform crs
  st_transform(crs = "EPSG:25832") %>%
  # Get only values inside the study area
...
## Export result
saveRDS(isochrone_pt, file = "isochrone_public_transport.rds")

```

```{r fig-isochrone-public, fig.cap="isochrone travel time from Kongens Nytorv (blue point) by public transport"}

isochrone_pt <- readRDS(file = "isochrone_public_transport.rds")

ggplot() +
  geom_sf(data = isochrone_pt, aes(fill = time), color = "grey50", size = 0.05) +
  scale_fill_viridis_c(name = "min", option = "magma", direction = -1) +
  geom_sf(data = cph_parish, fill = NA, color = "grey50", size = 0.05) +
  geom_sf(data = KN, color = "blue", shape = 16, size = 3) +
  theme_void() 
...
ee <- bind_rows(dd) %>% 
  mutate(feature = factor(feature, levels = c("walk", "byke", "car", "transit")))
...
saveRDS(ee, file = "isochrone_otp.rds")
