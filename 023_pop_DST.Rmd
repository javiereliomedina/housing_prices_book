...
1. Calculate the occupancy rate (*OR*) for the residential units of each parish (*j*): $$OR_{j} = \frac{pop_{j}}{N_{j}}$$
...
#' @param .year       Year of the analysis (e.g. 2019)
  ...
  f_pd_grids <- function(.year) {
    ...
    # Population at the first day of the year (.year)
    pop_year <- .pop %>%
      # Select population data  
      select(prsh_id, prsh_name, prsh_area_km2, date, pop_total) %>% 
...
We can therefore estimate the population density at the first day of the year by grid cells of 100m x 100m (figure \@ref(fig:fig-pop-den-grid)).
...
```{r fig-pop-den-grid, animation.hook="gifski", fig.cap = "Dasymetric map"}
...
```{r fig-pop-den-worldpop, fig.cap = "WorldPop total population density estimations in 2019"}
