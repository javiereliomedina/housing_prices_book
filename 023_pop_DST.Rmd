...
Plot population density at the last day of 2019 (first of 2010) by parish and ancestry (figure \@ref(fig:fig-pop-chor)).
```{r fig-pop-chor, fig.width = 9, fig.height = 7, fig.cap = "Choropleth map)"}

col <- brewer.pal(9, "YlGnBu")
pal <- colorRampPalette(col)
my_pallette <- pal(10)
  
pop_dens_choropleth <-  prsh_ances_sf %>% 
...
brks <- c(min(pop_dens_choropleth$value),
...
pop_dens_choropleth  %>% 
  ...
  labs(x = "",
       y = "",
       title = "Population density by parish",
...
f_pd_grids <- function(.pop = prsh_pop_sf,
                       .parish = cph_parish,
...
```{r fig-pop-den-grid, fig.width = 9, fig.height = 7, fig.cap = "Population density at the first day of 2019 by grid cells of 100m x 100m"}

pop_2019_g100m <-  f_pd_grids(.year = 2019)
