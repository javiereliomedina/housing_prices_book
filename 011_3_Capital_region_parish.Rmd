...
Population density by parish from 2008 to 2019. 

```{r fig-pop-parish, fig.height=10, fig.width=10, fig.cap = "Population density"}

capital_prsh_ancestry_sf %>% 
  ggplot() +
  geom_sf(data = dk_country_crop,
          fill = "grey") +
  geom_sf(aes(fill = pop_total_km2)) +
...
# NOTE: you may need to change the path to your file on "OneDrive - Aalborg Universitet" 
  ...
  plan(multisession, workers = 7)
  runits_oft_capital_read <- future_map_dfr(.x = csv_files_path, .f = f_res_units)
  plan("default")
...
The total number of residential units used for the analysis is therefore `r nrow(runits_oft_capital)` (Table \@ref(tab:tbl-runits-oft-capital)).
...
labelled::var_label(runits_oft_capital$price_2019_kDKK) <- "Adjusted prices (kDKK)"
labelled::var_label(runits_oft_capital$BEBO_ARL)           <- "Dwelling size (m2)"
labelled::var_label(runits_oft_capital$price_2019_kDKK_m2) <- "Adjusted prices per square meter (kDKK/m2)"
...
House prices by type (removing very low prices; i.e. <1 kDKK/m2, n = `r nrow(filter(runits_oft_capital, price_2019_kDKK_m2 < 1))`).
...
ggplot(data = filter(runits_oft_capital, price_2019_kDKK_m2 > 1),
