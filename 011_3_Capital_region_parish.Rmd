...
f1 <- function(year, variable, signif = 0.15) { 
  
  # Polygons
  s <- capital_prsh_ancestry_sf %>%
    # Select
    filter(year == {{ year }}) %>% 
    drop_na() %>% 
    st_sf() 
...
```{r fig-MI-nb, fig.height=25, fig.width=10, fig.cap="Local Moran index (p-value < 0.15)"}
...
Ternary maps: center over the compositional mean of 2019 (i.e. Danes ~ 80.3%; non-wst ~ 12.3; and wst ~ 7.4%).  
...
```{r tern-map-2018, fig.height=10, fig.width=10, fig.cap="Population distribution in 2018"}
...
remove one parish with non-western migrants up to 80% (NAME). There were not house prices in the ordinary free sale (only for one yesr); WHY?
...
ggtern::ggtern(data = sum_runits_oft_prices,
               aes(x = pop_dan,
                   y = pop_frgn_wst,
                   z = pop_frgn_nwst,
                   colour = cut_number(median_2019_kDKK_m2,
                                       n = 10,
                                       dig.lab = 0))) +
...
xc <- x %>% 
  clo(parts = c("pop_dan", "pop_frgn_nwst", "pop_frgn_wst"),
      total = 100) %>% 
  acomp()
```

PCA of clr transformations
```{r prsh-CoDa-PCA}
# PCA
pc <- princomp(xc)
summary(pc)
...
# Global index
moran.test(bal_sf$b1, listw = wts)
moran.test(bal_sf$b2, listw = wts)
...
LM_b1 <- bal_sf %>% 
  pull (b1) %>% 
    ...
    st_sf() %>% 
    # center the variable of interest around its mean
    mutate(m_qualification = b1 - mean( b1 ),
           m_local = Ii - mean(Ii)) %>% 
    # Build quadrant
    mutate(quadrant = case_when(m_qualification > 0 & m_local > 0 ~ 4,
                                m_qualification < 0 & m_local < 0 ~ 1,
...
plot(LM_b1["quadrant"], main = "LISA (B1)")
# plot(LM_b2["quadrant"], main = "LISA (B2)")
```
...
bal_sf %>% 
  ...
  mutate(name = gsub("pop_", "", name),
         name = gsub("_pct", "", name)) -> aa 
  

boxplot(value ~ name + CL, aa, notch = F)
