...
Where people live in the capital region (percentage over the total population) and their change from 2008 to 2019: 

```{r pop-parish-pct}

# Estimate percentage over the total population in a year 
capital_prsh_ancestry_sf <- capital_prsh_ancestry_sf %>% 
  group_by(year) %>% 
  mutate(across(.cols = c(pop_dan, pop_frgn_wst, pop_frgn_nwst),
                .fns = ~ 100 * .x / pop_total,
                .names = "{.col}_pct")) %>% 
  ungroup()
...
```{r fig-pop-parish, fig.height=10, fig.width=10, fig.cap = "Total population by parish"}

  capital_prsh_ancestry_sf %>% 
    ggplot() +
    geom_sf(data = dk_country_crop, fill = "grey") +
    geom_sf(aes(fill = pop_total ), color = "grey", size = 0.05) +
...
ggtern(data = capital_prsh_ancestry_sf,
       aes(x = pop_dan,
           y = pop_frgn_wst,
           z = pop_frgn_nwst,
           colour = factor(year)
...
```{r ternary-pop-parish, fig.height=20, fig.width=10, fig.cap = "ternary plot by parish (in 2019)"}
...
ex <- capital_prsh_ancestry_sf %>%
  filter(year == 2019) %>% 
  select(pop_dan, pop_frgn_wst, pop_frgn_nwst)
...
ex_map <- capital_prsh_ancestry_sf %>% 
  filter(year == 2019) %>% 
  mutate(pop_rgb = tric$rgb)

p <- ggplot(ex_map) +
  geom_sf(aes(fill = pop_rgb, geometry = geometry)) +
  scale_fill_identity() +
  my_theme_map() +
  coord_sf(datum = NA)
...
```{r fig-prices-runits-parish-a, animation.hook="gifski", fig.cap = "Median 2019-adjusted house prices in the ordinary free trade"}
...
```{r fig-prices-runits-parish-b,  fig.height=10, fig.width=10, fig.cap = "Median 2019-adjusted house prices in the ordinary free trade by grid cells"}
...
Median prices (kDKK/m2) in 2018
```{r fig-prices-runits-parish-2018,  fig.height=10, fig.width=10, fig.cap = "Median 2019-adjusted house prices in the ordinary free trade"}

sum_runits_oft_prices %>% 
  filter(year == 2018) %>% 
  ggplot() +
  geom_sf(data = dk_country_crop, fill = "grey") +
  geom_sf(aes(fill = cut_number(median_2019_kDKK_m2,
                                n = 10,
                                ordered_result = TRUE,
                                dig.lab = 0)),
          color = "grey",
          size = 0.05) +
  geom_sf(data = capital_muni, fill = NA, color = "white", size = 0.75) +
  scale_fill_viridis_d(name = TeX("$\\overset{\\textbf{Percentiles}}{(kDKK/m^2)}$"),
                       option = "magma",
                       direction = -1,
                       guide = guide_legend(reverse = TRUE) ) +
  ...
  labs(x = "",
       y = "",
       title = "Median housing prices in 2018") 
